...
export default function Playground({
  ...
  useEffect(() => {
    if (!didAutoConnect && roomState === ConnectionState.Disconnected) {
      onConnect(true);
      setDidAutoConnect(true);
    }
  }, [roomState, onConnect, didAutoConnect]);
  ...
  }, [roomState, localParticipant, config.settings.inputs.mic]);

  // Custom handler for disconnect button
  const handleDisconnect = () => {
    onConnect(false); // disconnect
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col grow basis-1/2 gap-4 max-h-[940px] ">
        {config.settings.chat && (
          <PlaygroundTile
            title="Chat"
            backgroundColor="gray-950"
            className="h-full grow hidden lg:flex"
          >
            {chatTileContent}
          </PlaygroundTile>
        )}
        ...
        {/*<div*/}
        {/*    className={`flex-col grow basis-1/4 gap-4 hidden lg:${*/}
